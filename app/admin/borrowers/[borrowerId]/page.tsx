...
import { EditLoanDescriptionForm } from "@/components/admin/EditLoanDescriptionForm";
...
export default async function BorrowerDetailPage({ params, searchParams }: { params: { borrowerId: string }; searchParams: Record<string, string | undefined> }) {
  ...
  const pageUrl = (updates: Record<string, number>) => { const query=new URLSearchParams(); if(currentLoanPage>1)query.set("loanPage",String(currentLoanPage)); Object.entries(updates).forEach(([key,value])=>value>1?query.set(key,String(value)):query.delete(key)); const params=query.toString(); return params?`/admin/borrowers/${borrower.id}?${params}`:`/admin/borrowers/${borrower.id}`; };
  ...
  return <div className="space-y-8">
    ...
    <Card className="overflow-hidden"><div className="h-1.5 bg-gradient-to-r from-indigo-500 via-violet-500 to-fuchsia-500"/><CardContent className="p-6 sm:p-8"><div className="flex flex-col gap-6 xl:flex-row xl:items-start xl:justify-between"><div className="flex min-w-0 items-start gap-4"><span className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-accent text-accent-foreground"><UserRound className="h-6 w-6"/></span><div className="min-w-0"><p className="text-xs font-semibold uppercase tracking-[.16em] text-primary">Profil peminjam</p><h1 className="mt-1 truncate text-3xl font-bold sm:text-4xl">{borrower.name}</h1><p className="mt-2 flex items-center gap-2 text-sm text-muted-foreground"><Phone className="h-4 w-4" />{borrower.phone || "Nomor WhatsApp belum diisi"}</p>{borrower.tags.length>0&&<div className="mt-3 flex flex-wrap gap-2">{borrower.tags.map(tag=><span key={tag} className="rounded-full bg-violet-500/10 px-3 py-1 text-xs font-semibold text-violet-300 ring-1 ring-violet-500/20">{tag}</span>)}</div>}</div></div><div className="flex flex-wrap gap-2"><EditBorrowerForm borrower={{ id: borrower.id, name: borrower.name, phone: borrower.phone, notes: borrower.notes, tags: borrower.tags }} /><BorrowerActions borrowerId={borrower.id} /></div></div><div className="mt-7 grid gap-4 border-t pt-6 lg:grid-cols-2"><div className="rounded-xl bg-muted/60 p-4"><div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-wider text-muted-foreground"><KeyRound className="h-3.5 w-3.5"/>Kode akses peminjam</div><div className="mt-3 flex flex-wrap items-center justify-between gap-4"><p className="font-mono text-2xl font-bold tracking-[.28em]">{verifyCode || "Belum diatur"}</p><CopyVerificationCode code={verifyCode} borrowerId={borrower.id}/></div></div><div className="rounded-xl bg-amber-500/10 p-4"><div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-wider text-amber-300"><StickyNote className="h-3.5 w-3.5"/>Catatan pribadi</div><p className="mt-3 text-sm leading-6 text-amber-100">{borrower.notes || "Belum ada catatan pribadi."}</p></div></div></CardContent></Card>

    <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><Card><CardContent className="p-5"><p className="text-sm font-medium text-muted-foreground">Jumlah pinjaman</p><p className="mt-2 text-3xl font-bold">{borrower.loans.length}</p></CardContent></Card><Card><CardContent className="p-5"><p className="text-sm font-medium text-muted-foreground">Total gabungan</p><p className="mt-2 text-xl font-bold">{formatRupiah(totalAmount)}</p></CardContent></Card><Card><CardContent className="p-5"><p className="text-sm font-medium text-muted-foreground">Sudah dibayar</p><p className="mt-2 text-xl font-bold text-emerald-400">{formatRupiah(paidAmount)}</p></CardContent></Card><Card className="border-indigo-500/20 bg-indigo-500/10"><CardContent className="p-5"><p className="text-sm font-medium text-muted-foreground">Sisa gabungan</p><p className="mt-2 text-xl font-bold text-indigo-300">{formatRupiah(remainingAmount)}</p></CardContent></Card></div>
