...
import { Banknote, CircleCheckBig, Inbox, Plus, SearchX, Tag as TagIcon, UsersRound, X } from "lucide-react";
import { getAllBorrowers } from "@/lib/data";
import { calculateBorrowerStatus, formatLoanDuration, formatRupiah, normalizeBorrowerName, statusLabel } from "@/lib/utils";
...
export default async function Dashboard({ searchParams }: { searchParams: { status?: string; tag?: string; page?: string; q?: string } }) {
  ...
  const borrowers = all.filter(borrower => {
    if (filter !== "all" && calculateBorrowerStatus(borrower.loans) !== filter) return false;
    if (selectedTag !== "all" && !borrower.tags.includes(selectedTag)) return false;
    if (searchQuery && !normalizeBorrowerName(borrower.name).includes(searchQuery)) return false;
    return true;
  });
  const perPage = 5;
  const totalPages = Math.max(1, Math.ceil(borrowers.length / perPage));
  const currentPage = Math.min(totalPages, Math.max(1, Number(searchParams.page) || 1));
  const paginatedBorrowers = borrowers.slice((currentPage - 1) * perPage, currentPage * perPage);
  ...
  const dashboardUrl = (status: string, tag: string, page = 1) => { const params=new URLSearchParams(); if(status!=="all")params.set("status",status); if(tag!=="all")params.set("tag",tag); if(page>1)params.set("page",String(page)); const query=params.toString(); return query?`/admin/dashboard?${query}`:"/admin/dashboard"; };
  ...
  return <div className="space-y-8">
    ...
    <section className="space-y-5"><div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><h2 className="text-xl font-bold">Daftar peminjam</h2><p className="mt-1 text-sm text-muted-foreground">{searchQuery ? `${borrowers.length} hasil untuk "${searchQuery}"` : `${borrowers.length} data ditampilkan`}{selectedTag !== "all" && searchQuery && ` · `}{selectedTag !== "all" && <span className="inline-flex items-center gap-1">Tag: <span className="font-medium text-foreground">{selectedTag}</span></span>}</p></div><SearchInput initialQuery={searchParams.q || ""} /></div><Card className="p-2"><div className="flex gap-1 overflow-x-auto">{tabs.map(([key, label]) => <Link key={key} href={dashboardUrl(key,selectedTag)} className={`whitespace-nowrap rounded-lg px-4 py-2.5 text-sm font-semibold transition ${filter === key ? "bg-primary text-white shadow-sm" : "text-muted-foreground hover:bg-muted hover:text-foreground"}`}>{label}</Link>)}</div></Card>{tags.length>0&&<div className="flex items-center gap-2 overflow-x-auto pb-1"><span className="shrink-0 text-xs font-semibold uppercase tracking-wider text-muted-foreground">Tag</span><Link href={dashboardUrl(filter,"all")} className={`whitespace-nowrap rounded-full border px-3 py-1.5 text-xs font-semibold transition ${selectedTag==="all"?"border-primary bg-accent text-accent-foreground":"bg-card text-muted-foreground hover:border-primary/30"}`}>Semua</Link>{tags.map(tag=><Link key={tag} href={dashboardUrl(filter,tag)} className={`whitespace-nowrap rounded-full border px-3 py-1.5 text-xs font-semibold transition ${selectedTag===tag?"border-primary bg-accent text-accent-foreground":"bg-card text-muted-foreground hover:border-primary/30"}`}>{tag}</Link>)}</div>}</section>
    {selectedTag !== "all" && <Card className="overflow-hidden border-violet-500/20 bg-gradient-to-br from-card via-card to-violet-500/10"><CardContent className="p-0"><div className="flex flex-col gap-4 border-b border-violet-500/20 px-5 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-6"><div className="flex items-center gap-3"><span className="flex h-10 w-10 items-center justify-center rounded-xl bg-violet-500/10 text-violet-300"><TagIcon className="h-4 w-4"/></span><div><p className="text-xs font-semibold uppercase tracking-[.16em] text-violet-400">Rincian tag aktif</p><h3 className="mt-0.5 text-lg font-bold">{selectedTag}</h3></div></div><Link href={dashboardUrl(filter,"all")} className="inline-flex items-center gap-1.5 self-start rounded-lg px-3 py-2 text-xs font-semibold text-muted-foreground transition hover:bg-accent hover:text-foreground sm:self-auto"><X className="h-3.5 w-3.5"/>Hapus filter tag</Link></div><div className="grid lg:grid-cols-[1.25fr_2fr]"><div className="border-b border-violet-500/20 p-5 sm:p-6 lg:border-b-0 lg:border-r"><p className="flex items-center gap-2 text-sm font-medium text-muted-foreground"><Banknote className="h-4 w-4 text-violet-400"/>Sisa piutang tag ini</p><p className="mt-3 text-3xl font-bold text-violet-300 sm:text-4xl">{formatRupiah(tagRemainingAmount)}</p><p className="mt-2 text-xs leading-5 text-muted-foreground">Total sisa seluruh pinjaman dari peminjam dengan tag “{selectedTag}”.</p></div><div className="grid grid-cols-2 divide-x divide-y divide-violet-500/20 sm:grid-cols-3 sm:divide-y-0"><div className="p-5 sm:p-6"><p className="text-xs font-medium text-muted-foreground">Total pinjaman</p><p className="mt-2 text-lg font-bold">{formatRupiah(tagTotalAmount)}</p></div><div className="p-5 sm:p-6"><p className="flex items-center gap-1.5 text-xs font-medium text-muted-foreground"><CircleCheckBig className="h-3.5 w-3.5 text-emerald-400"/>Sudah dibayar</p><p className="mt-2 text-lg font-bold text-emerald-400">{formatRupiah(tagPaidAmount)}</p></div><div className="col-span-2 p-5 sm:col-span-1 sm:p-6"><p className="text-xs font-medium text-muted-foreground">Cakupan data</p><p className="mt-2 text-lg font-bold">{taggedBorrowers.length} peminjam</p><p className="mt-1 text-xs text-muted-foreground">{tagLoanCount} pinjaman</p></div></div></div></CardContent></Card>}
    {borrowers.length === 0 ? <Card><CardContent className="flex flex-col items-center px-6 py-16 text-center">{searchQuery ? <><span className="mb-4 flex h-14 w-14 items-center justify-center rounded-2xl bg-muted text-muted-foreground"><SearchX className="h-6 w-6"/></span><h3 className="font-bold">Peminjam tidak ditemukan</h3><p className="mt-1 max-w-sm text-sm text-muted-foreground">Tidak ada peminjam dengan nama "{searchQuery}". Coba kata kunci lain atau periksa ejaan.</p></> : <><span className="mb-4 flex h-14 w-14 items-center justify-center rounded-2xl bg-muted text-muted-foreground"><Inbox className="h-6 w-6"/></span><h3 className="font-bold">Belum ada data</h3><p className="mt-1 max-w-sm text-sm text-muted-foreground">Tidak ada peminjam pada filter ini. Coba filter lain atau tambahkan peminjam baru.</p></>}</CardContent></Card> : <>
      <div className="grid gap-4 md:hidden">{paginatedBorrowers.map(borrower => <BorrowerCard key={borrower.id} borrower={borrower} />)}</div>
      <Card className="hidden overflow-hidden md:block"><CardContent className="p-0"><Table><TableHeader className="bg-muted/50"><TableRow><TableHead className="pl-6">Peminjam</TableHead><TableHead>Pinjaman</TableHead><TableHead>Total gabungan</TableHead><TableHead>Sisa gabungan</TableHead><TableHead>Status</TableHead><TableHead className="pr-6 text-right">Aksi</TableHead></TableRow></TableHeader><TableBody>{paginatedBorrowers.map(borrower => {
