...
export function ProofUploadForm({borrowerId,loanId,role,shareToken}:Props){
  ...
  return <form onSubmit={submit} className="space-y-3 rounded-lg border bg-slate-50 p-4"><div><h4 className="font-medium">Upload bukti transfer <span className="font-normal text-muted-foreground">(opsional)</span></h4><p className="text-xs text-muted-foreground">JPG, PNG, WebP, atau PDF · maksimal 3 MB.</p></div><div className="grid gap-3 sm:grid-cols-2"><div className="space-y-2"><Label htmlFor={`proof-${role}-${loanId}`}>File bukti</Label><Input id={`proof-${role}-${loanId}`} name="file" type="file" accept="image/jpeg,image/png,image/webp,application/pdf" required/></div><div className="space-y-2"><Label htmlFor={`caption-${role}-${loanId}`}>Keterangan</Label><Input id={`caption-${role}-${loanId}`} name="caption" maxLength={200} placeholder="Contoh: Transfer cicilan Mei"/></div></div>{message&&<p className={`rounded-md p-3 text-sm ${message.ok?"bg-emerald-50 text-emerald-700":"bg-red-50 text-red-700"}`}>{message.text}</p>}<Button disabled={loading} size="sm">{loading?<Loader2 className="h-4 w-4 animate-spin"/>:<FileUp className="h-4 w-4"/>}{loading?"Mengunggah...":"Upload Bukti"}</Button></form>;
}
