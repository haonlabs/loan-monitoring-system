...
export function BorrowerCard({ borrower }: { borrower: BorrowerWithLoans }) {
  ...
  return <Card className="overflow-hidden"><CardContent className="space-y-5 p-5">
    <div className="flex items-start justify-between gap-3"><div><p className="font-semibold">{borrower.name}</p>{borrower.tags.length>0&&<div className="mt-1 flex flex-wrap gap-1">{borrower.tags.map(tag=><span key={tag} className="rounded-full bg-violet-100 px-2 py-0.5 text-[11px] font-medium text-violet-700">{tag}</span>)}</div>}<p className="mt-1 text-sm text-muted-foreground">{borrower.loans.length} pinjaman{latest ? ` · terbaru ${formatLoanDuration(latest.startDate)}` : ""}</p></div><Badge variant={status === "paid" ? "paid" : status === "overdue" ? "overdue" : "default"}>{statusLabel(status)}</Badge></div>
    ...
    <div className="flex items-center gap-2 border-t pt-4"><Button asChild size="sm" className="flex-1"><Link href={`/admin/borrowers/${borrower.id}`}>Lihat detail<ChevronRight className="h-4 w-4"/></Link></Button><ShareButton compact borrowerName={borrower.name} shareToken={borrower.shareToken} phoneNumber={borrower.phone} /></div>
  </CardContent></Card>;
}
