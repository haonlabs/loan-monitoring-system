...
export function BorrowerCard({ borrower }: { borrower: BorrowerWithLoans }) {
  ...
  return <Card><CardContent className="space-y-4 p-5">
    ...
    <div className="flex flex-wrap gap-2"><Button asChild size="sm"><Link href={`/admin/borrowers/${borrower.id}`}>Lihat {borrower.loans.length} pinjaman</Link></Button><ShareButton compact borrowerName={borrower.name} shareToken={borrower.shareToken} phoneNumber={borrower.phone} /></div>
  </CardContent></Card>;
}
