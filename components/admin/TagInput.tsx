...
export function TagInput({ name = "tags", defaultValue = [] }: { name?: string; defaultValue?: string[] }) {
  ...
  return <div className="space-y-2">
    ...
    {tags.length > 0 && <div className="flex flex-wrap gap-2">{tags.map(tag => <span key={tag} className="inline-flex items-center gap-1 rounded-full bg-violet-100 px-3 py-1 text-xs font-medium text-violet-700">{tag}<button type="button" onClick={() => setTags(items => items.filter(item => item !== tag))} aria-label={`Hapus tag ${tag}`} className="rounded-full hover:bg-violet-200"><X className="h-3 w-3" /></button></span>)}</div>}
