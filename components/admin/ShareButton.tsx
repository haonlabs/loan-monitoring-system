...
export function ShareButton({ borrowerName, shareToken, verifyCode, phoneNumber, compact }: ShareButtonProps) {
  ...
  return <div className="flex flex-wrap gap-2"><Button type="button" variant="outline" size={compact?"sm":"default"} onClick={copy}>{copied?<Check className="h-4 w-4 text-emerald-400"/>:<Copy className="h-4 w-4"/>}{copied?"Tersalin!":compact?"Salin":"Salin Pesan"}</Button><Button type="button" variant="outline" size={compact?"sm":"default"} onClick={whatsapp}><MessageCircle className="h-4 w-4 text-emerald-400" />{compact?"WhatsApp":"Kirim via WhatsApp"}</Button></div>;
}
